...
class Person {
  ...
  age: number;
  birthDate: string;
  programmingLanguages: string[];s

  constructor(name: string, age: number, birthDate: string, programmingLanguages: string[]) {
    this.name = name;
    this.age = age;
  ...
  static fromJSON(jsonFileName: string): Person {
  ...
  static fromXML(xmlFileName: string): Person {
    const xmlData = fs.readFileSync(xmlFileName, 'utf8');
    const name = xmlData.match(/<name>(.*?)<\/name>/)?.[1] || '';
    const age = parseInt(xmlData.match(/<age>(.*?)<\/age>/)?.[1] || '0', 10);
    const birthDate = xmlData.match(/<birthDate>(.*?)<\/birthDate>/)?.[1] || '';
    const programmingLanguages = (xmlData.match(/<language>(.*?)<\/language>/g) || [])
      .map((lang: string) => lang.replace(/<\/?language>/g, ''));
